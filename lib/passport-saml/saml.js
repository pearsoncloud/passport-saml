...
SAML.prototype.generateInstant = function() {
    var date = new Date();
    var hour = ('0' + (date.getUTCHours() + 2)).slice(-2);
    if (hour == '24') hour = '00';
    if (hour == '25') hour = '01';
    return date.getUTCFullYear() + '-' + ('0' + (date.getUTCMonth() + 1)).slice(-2) + '-' + ('0' + date.getUTCDate()).slice(-2) + 'T' + hour + ":" + ('0' + date.getUTCMinutes()).slice(-2) + ":" + ('0' + date.getUTCSeconds()).slice(-2) + "Z";
};
 ...
 */
SAML.prototype.signXml = function(xml, xpathToLocation, xpathToId, key) {
    var sig = new SignedXml();
    sig.signingKey = key;
    sig.addReference(xpathToId);
    sig.computeSignature(xml, xpathToLocation);
...
SAML.prototype.generateAuthorizeRequest = function(req) {
    ...
    if (this.options.privateCert) {
        request = this.signXml(request, "//*[local-name(.)='Issuer']", "//*[local-name(.)='AuthnRequest']", this.options.privateCert);
        this.log.debug("Signed login request: " + request);
    }
...
SAML.prototype.generateLogoutRequest = function(req) {
    ...
    if (this.options.privateCert) {
        request = this.signXml(request, "//*[local-name(.)='Issuer']", "//*[local-name(.)='LogoutRequest']", this.options.privateCert);
        this.log.debug("Signed logout request: " + request);
    }
...
SAML.prototype.validateResponse = function(samlResponse, callback) {
    ...
    self.log.debug('validating SAML response');

    var xml = new Buffer(samlResponse, 'base64').toString('ascii');
    var x = xml.toString();

    if (x.indexOf('<samlp:Response') !== 0)
        return callback(new Error('Invalid xml response'), null, false);
    ...
    parser.parseString(xml, function(err, doc) {
        ...
        if (response) {
            ...
            // Sets isMegatron to true if user profile includes a super admin or org creator
            if (typeof profile.isSuper === 'string' && profile.isSuper.indexOf("myStackSuperAdmins") >= 0)
                profile.isMegatron = true;
            else
                profile.isMegatron = false;
